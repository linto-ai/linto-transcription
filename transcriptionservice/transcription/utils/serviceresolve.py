...
class FailedToResolve(ResolveException):
    ...
    def __init__(self, service_name: str, policy: str) -> None:
        self.message = (
            f"The required service {service_name} could not be resolved. (Policy={policy})."
        )
        logging.error(self.message)
        super().__init__(self.message)
...
class NoDefaultDeclared(ResolveException):
    ...
    def __init__(self, service_type: str, policy: str) -> None:
        self.message = f"The default service for {service_type} is not set (Policy={policy})."
        logging.error(self.message)
        super().__init__(self.message)
...
class ServiceResolver:
    """The ServiceResolver class is used to fetch available services and resolve service configuration from transcrition requests."""
    ...
    def resolve_task(self, task_config: "TaskConfig") -> bool:
        ...
        if not task_config.isEnabled:
            logging.debug(f"Service {task_config.task_name} is not enabled, no resolve necessary.")
            return True
        ...
        task_config.setService(resolving_service.service_name, resolving_service.queue_name)
    ...
    def _resolve_default(self, service_type: str) -> "Service":
        ...
        if default_service_name:
            if default_service_name in self.subservices_list[service_type]:
                ...
                raise DefaultUnavailable(default_service_name, service_type, self.service_policy)
        else:
            raise NoDefaultDeclared(service_type=service_type, policy=self.service_policy)
